...
from cbor_json import (
    cbor_from_jsonable,
    jsonable_from_cbor,
    native_from_jsonable,
    native_from_cbor,
    cbor_from_native,
    jsonable_from_native,
    SerializableToCbor,
...
def test_vectors():
    ...
    for idx, vector in enumerate(vectors):
        ...
        #
        # cbor1_b -(jfc)→ jsonable1 → json -(nfj)→ native1 -(cfn)→ cbor2_b →
        #    check roundtrip -(nfc)→ native2 -(jfn)→ jsonable2 -(cfj)→ cbor3_b →
        ...
        if cbor2_b != cbor1_b and vector["roundtrip"]:
            ...
            if (
                isinstance(native1, float)
                and isinstance(native_x, float)
                and round(native1 * 1000) == round(native_x * 1000)
            ):
                print("  ...it's float")
            elif (
                isinstance(native1, datetime)
                and isinstance(native_x, datetime)
    ...
    for case_name, case_data in ext_jsons.items():
        ...
        if "assert" in case_data:
            if not eval(case_data["assert"]):
                print(
                    f'{case_name}: "assert" failed; native={native} '
...
def test_rising() -> None:
    with pytest.raises(ValueError) as exc_ve:
        cbor_from_native(test_rising)
    assert str(exc_ve.value) == "Cannot convert function to cborable format"

    with pytest.raises(ValueError) as exc_ve:
        _native_from_cborable(test_rising)
    assert str(exc_ve.value) == "Cannot convert function to native format"

    with pytest.raises(ValueError) as exc_ve:
        _transform_collection(1, 2, 3)  # absurdic, but anyway...
    assert str(exc_ve.value) == "Convestion for int is not implemented"
    ...
    with pytest.raises(ValueError) as exc_ve:
        cbor_from_native(a1)
    assert str(exc_ve.value) == "Cannot encode a recursively linked structure"
    ...
    assert str(exc_ve.value) == '$type "haha" is not supported'

    with pytest.raises(TypeError) as exc_te:
        cbor_from_jsonable(test_rising)
    assert str(exc_te.value) == "Value of type function is not JSONable"

    with pytest.raises(ValueError) as exc_ve:
        register_custom_class(datetime)  # type: ignore
    assert str(exc_ve.value) == "Class datetime is not a subclass of SerializableToCbor"

    with pytest.raises(ValueError) as exc_ve:
        register_custom_class(WrongSerializable1)
    assert str(exc_ve.value) == "Class tag is not defined for class WrongSerializable1"

    with pytest.raises(ValueError) as exc_ve:
        register_custom_class(WrongSerializable2)
    assert str(exc_ve.value) == (
